...
class agent():
    def __init__(self, n_machines, policy='softmax', policy_param=.1):
        ...
        self.machines = np.zeros(n_machines)
...
def experiment(n_runs, n_machines, n_steps, method, param):
    rewards = np.zeros((n_runs, n_steps))
    optimals = np.zeros((n_runs, n_steps))

    for run in range(n_runs):
        env = environment(n_machines)
        aj = agent(n_machines, policy=method, policy_param=param)
...
def main():
    ...
    n_machines = 10
    n_steps = 1000
    parameters = [('uniform', 0), ('uniform', .1), ('softmax', .2)]
    avg_rewards, optimal_actions = [], []
    legends = []

    for m, p in parameters:
        r, a = experiment(n_runs, n_machines, n_steps, method=m, param=p)
