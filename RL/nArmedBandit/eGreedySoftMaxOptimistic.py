...
class agent():
    def __init__(self, machines, method='uniform', exploration=0, temperature=.1, optimistic=.0):
        self.method = method
        self.exploration = exploration
        self.temperature = temperature
        # creats an estimate of n machines
        self.machines = np.ones(machines) * optimistic
...
def experiment(n_runs, n_machines, n_steps, method, exploration, temperature, optimistic):
    rewards = np.zeros((n_runs, n_steps))
    optimals = np.zeros((n_runs, n_steps))

    for run in range(n_runs):
        env = environment(n_machines)
        aj = agent(n_machines, method, exploration, temperature, optimistic)
...
def main():
    ...
    for m, e, t in parameters:
        r, a = experiment(n_runs, n_machines, n_steps, method=m, exploration=e, temperature=t, optimistic=0.0)
        ...
        r, a = experiment(n_runs, n_machines, n_steps, method=m, exploration=e, temperature=t, optimistic=5)
        avg_rewards.append(r)
        optimal_actions.append(a)
        legends.append('{0} exploration= {1}% {2}(optimistic)'.format(m, 100*e, '' if t is None else 'temperature= {0} '.format(t)))
