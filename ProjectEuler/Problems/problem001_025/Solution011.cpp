...
08  2 22 97 38 15  0 40  0 75  4  5  7 78 52 12 50 77 91 08
...
int getHorizontal(int grid[20][20]) {
	...
	for (int r = 0; r < 20; r++) {
		...
		if (!zeros && mul > max)
			max = mul;
		for (int i = 4; i < 20; i++) {
			if (grid[r][i] && grid[r][i - 4]) {
				mul = (mul / grid[r][i - 4]) * grid[r][i];
			}
			else if (grid[r][i]) {
				mul *= grid[r][i];
				zeros--;
...
int getDiagonal(int grid[20][20]) {
	int max = 0;
	for (int d = 3; d < 2 * 20 - 4; d++) {
		int x = (d < 20) ? 0 : d - 19;
		int y = (d < 20) ? 19 - d : 0;
		int l = (d < 20) ? d + 1 : 39 - d;
		int zeros = 0, mul = 1;
		...
		if (!zeros && mul > max)
			max = mul;
		for (int i = 4; i < l; i++) {
			if (grid[y + i][x + i] && grid[y + i - 4][x + i - 4]) {
				mul = (mul / grid[y + i - 4][x + i - 4]) * grid[y + i][x + i];
			}
			else if (grid[y + i][x + i]) {
				mul *= grid[y + i][x + i];
				zeros--;
			}
			else if (grid[y + i - 4][x + i - 4]) {
				mul /= grid[y + i - 4][x + i - 4];
				zeros++;
			}
			if (!zeros && mul > max)
				max = mul;
